...
- **Social_Auth**: Authentication via Google identity tokens, verified against Google's public keys, used to create-or-find a user by email.
...
### Requirement 4: Email Existence Check
...
1. ALL successful auth responses (register, login, Google auth) SHALL return HTTP 200 with the JSON body `{ status: "ok", token: "<sanctum_token>", athlete: "<users.name>", email: "<users.email>" }`
...
### Requirement 6: Exercise Resolution — Accept canonical_name
...
### Requirement 7: Merge Operation — Store canonical_name as alias
...
### Requirement 8: Exercise Resolution — Global-Only Scope
