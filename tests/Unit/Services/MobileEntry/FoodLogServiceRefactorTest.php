...
class FoodLogServiceRefactorTest extends TestCase
{
    ...
    /** @test */
    public function generate_forms_returns_empty_array()
    {
        $forms = $this->service->generateForms($this->user->id, Carbon::now(), request());
    ...
    /** @test */
    public function generate_ingredient_form_returns_null()
    {
        $ingredient = Ingredient::factory()->create(['user_id' => $this->user->id]);

        $form = $this->service->generateIngredientForm($this->user->id, $ingredient->id, Carbon::now());

        $this->assertNull($form);
    }

    /** @test */
    public function generate_meal_form_returns_null()
    {
        $meal = Meal::factory()->create(['user_id' => $this->user->id]);

        $form = $this->service->generateMealForm($this->user->id, $meal->id, Carbon::now());

        $this->assertNull($form);
    }

    /** @test */
    public function add_food_form_returns_error_message()
    {
        $ingredient = Ingredient::factory()->create(['user_id' => $this->user->id]);

        $result = $this->service->addFoodForm($this->user->id, 'ingredient', $ingredient->id, Carbon::now());

        $this->assertIsArray($result);
        $this->assertArrayHasKey('message', $result);
        $this->assertStringContainsString('deprecated', $result['message']);
        $this->assertFalse($result['success']);
    }

    /** @test */
    public function remove_food_form_returns_error_message()
    {
        $result = $this->service->removeFoodForm($this->user->id, 1);
    ...
    /** @test */
    public function cleanup_old_forms_is_no_op()
    {
        // This should not throw any exceptions
        $this->service->cleanupOldForms($this->user->id, Carbon::now());
    ...
    /** @test */
    public function remove_form_after_logging_returns_true()
    {
        $result = $this->service->removeFormAfterLogging($this->user->id, 'ingredient', 1, Carbon::now());
