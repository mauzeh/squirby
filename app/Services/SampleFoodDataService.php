...
class SampleFoodDataService
{
    /**
     * Create sample food data (50 ingredients and 5 meals) for a user.
     *
     * @param User $user
     * @return array ['ingredients' => Collection, 'meals' => Collection]
     */
    public function createSampleData(User $user): array
    {
        // Get common units
        ...
        if (!$gramUnit || !$cupUnit || !$tbspUnit || !$pieceUnit || !$servingUnit) {
            throw new \Exception('Required units not found. Please ensure units are seeded.');
        }

        $ingredients = $this->createIngredients($user, $gramUnit, $cupUnit, $tbspUnit, $pieceUnit, $servingUnit);
        $meals = $this->createSampleMeals($user, $ingredients);
    ...
    /**
     * Create 50 common ingredients for the user.
     */
    ...
    /**
     * Create 5 sample meals for the user.
     */
