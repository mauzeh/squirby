        ...
        <div class="form-container">
            <form action="{{ route('workout-programs.store') }}" method="POST" id="create-program-form">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $selectedDate->format('Y-m-d')) }}" required>
                    @error('date')
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" id="notes" rows="3" placeholder="General program notes...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div id="exercises-container">
                    @php
                        $oldExercises = old('exercises', [['exercise_id' => '', 'exercise_type' => '', 'sets' => '', 'reps' => '', 'notes' => '']]);
                    @endphp
                    
                    @foreach($oldExercises as $index => $oldExercise)
                        <div class="exercise-entry" data-index="{{ $index }}">
                            ...
                            <div class="form-group">
                                ...
                                <select name="exercises[{{ $index }}][exercise_id]" id="exercises_{{ $index }}_exercise_id" required>
                                    ...
                                    @foreach ($exercises as $exercise)
                                        <option value="{{ $exercise->id }}" {{ old('exercises.' . $index . '.exercise_id') == $exercise->id ? 'selected' : '' }}>
                                            {{ $exercise->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="exercises[{{ $index }}][exercise_type]" id="exercises_{{ $index }}_exercise_type" required>
                                    ...
                                    <option value="main" {{ old('exercises.' . $index . '.exercise_type') == 'main' ? 'selected' : '' }}>Main Lift</option>
                                    <option value="accessory" {{ old('exercises.' . $index . '.exercise_type') == 'accessory' ? 'selected' : '' }}>Accessory</option>
                                </select>
                            ...
                            <div class="form-row">
                                <div class="form-group" style="flex: 1; margin-right: 1rem;">
                                    ...
                                    <input type="number" name="exercises[{{ $index }}][sets]" id="exercises_{{ $index }}_sets" 
                                           min="1" max="20" value="{{ old('exercises.' . $index . '.sets') }}" required>
                                ...
                                <div class="form-group" style="flex: 1;">
                                    ...
                                    <input type="number" name="exercises[{{ $index }}][reps]" id="exercises_{{ $index }}_reps" 
                                           min="1" max="100" value="{{ old('exercises.' . $index . '.reps') }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="exercises[{{ $index }}][notes]" id="exercises_{{ $index }}_notes" 
                                       value="{{ old('exercises.' . $index . '.notes') }}" 
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('workout-programs.index', ['date' => $selectedDate->format('Y-m-d')]) }}" class="button">Cancel</a>
                </div>
            </form>
        </div>
    </div>
