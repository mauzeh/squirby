...
@section('content')
    <div class="container">
        <h1>Add Intelligence Data</h1>
        <h2 style="color: #aaa; margin-top: 0;">{{ $exercise->title }}</h2>
        ...
        <form action="{{ route('exercise-intelligence.store', $exercise) }}" method="POST" id="intelligence-form">
            @csrf
            ...
            <div class="form-container">
                ...
                <div class="form-group">
                    ...
                    <select name="movement_archetype" id="movement_archetype" class="form-control" required>
                        ...
                        <option value="push" {{ old('movement_archetype') == 'push' ? 'selected' : '' }}>Push - Pushing movements (bench press, overhead press, push-ups)</option>
                        <option value="pull" {{ old('movement_archetype') == 'pull' ? 'selected' : '' }}>Pull - Pulling movements (rows, pull-ups, deadlifts)</option>
                        <option value="squat" {{ old('movement_archetype') == 'squat' ? 'selected' : '' }}>Squat - Knee-dominant lower body (squats, lunges)</option>
                        <option value="hinge" {{ old('movement_archetype') == 'hinge' ? 'selected' : '' }}>Hinge - Hip-dominant movements (deadlifts, hip thrusts)</option>
                        <option value="carry" {{ old('movement_archetype') == 'carry' ? 'selected' : '' }}>Carry - Loaded carries and holds (farmer's walks)</option>
                        <option value="core" {{ old('movement_archetype') == 'core' ? 'selected' : '' }}>Core - Core-specific movements (planks, crunches)</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="category" id="category" class="form-control" required>
                        ...
                        <option value="strength" {{ old('category') == 'strength' ? 'selected' : '' }}>Strength - Traditional resistance training</option>
                        <option value="cardio" {{ old('category') == 'cardio' ? 'selected' : '' }}>Cardio - Cardiovascular exercises</option>
                        <option value="mobility" {{ old('category') == 'mobility' ? 'selected' : '' }}>Mobility - Flexibility and mobility work</option>
                        <option value="plyometric" {{ old('category') == 'plyometric' ? 'selected' : '' }}>Plyometric - Explosive, jumping movements</option>
                        <option value="flexibility" {{ old('category') == 'flexibility' ? 'selected' : '' }}>Flexibility - Static stretching exercises</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-container">
                ...
                <div class="form-group">
                    ...
                    <select name="difficulty_level" id="difficulty_level" class="form-control" required>
                        ...
                        <option value="1" {{ old('difficulty_level') == '1' ? 'selected' : '' }}>1 - Beginner (very easy to learn and perform)</option>
                        <option value="2" {{ old('difficulty_level') == '2' ? 'selected' : '' }}>2 - Novice (easy with basic instruction)</option>
                        <option value="3" {{ old('difficulty_level') == '3' ? 'selected' : '' }}>3 - Intermediate (moderate complexity)</option>
                        <option value="4" {{ old('difficulty_level') == '4' ? 'selected' : '' }}>4 - Advanced (requires good technique)</option>
                        <option value="5" {{ old('difficulty_level') == '5' ? 'selected' : '' }}>5 - Expert (very complex, high skill required)</option>
                    </select>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Add initial muscle row
            addMuscleRow();

            // Restore old values if validation failed
            @if(old('muscle_data.muscles'))
                @foreach(old('muscle_data.muscles') as $index => $muscle)
                    @if($index > 0)
                        addMuscleRow('{{ $muscle['name'] ?? '' }}', '{{ $muscle['role'] ?? '' }}', '{{ $muscle['contraction_type'] ?? '' }}');
                    @else
                        // Update first row
                        const firstRow = document.querySelector('.muscle-row');
                        if (firstRow) {
                            firstRow.querySelector('.muscle-select').value = '{{ $muscle['name'] ?? '' }}';
                            firstRow.querySelector('.muscle-role').value = '{{ $muscle['role'] ?? '' }}';
                            firstRow.querySelector('select[name*="contraction_type"]').value = '{{ $muscle['contraction_type'] ?? '' }}';
                        }
                    @endif
                @endforeach
                updatePrimaryMoverOptions();
